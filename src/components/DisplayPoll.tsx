...
export default (props: Record<keyof any, any>) => {
	...
	const firstPercentage = createMemo(() => Math.round((firstOptionVotes() / (totalVotes() || 1)) * 100));
	const secondPercentage = createMemo(() => Math.round((secondOptionVotes() / (totalVotes() || 1)) * 100));
	const thirdPercentage = createMemo(() => Math.round((thirdOptionVotes() / (totalVotes() || 1)) * 100));
	const fourthPercentage = createMemo(() => Math.round((fourthOptionVotes() / (totalVotes() || 1)) * 100));
