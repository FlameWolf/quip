					...
					<p class="card-text">
						...
						<Show when={typeof attachedPost === "string"}>
							...
							<Show when={attachedPost !== nullId} fallback={<div class="text-bg-secondary border rounded p-3">The quoted post is not available.</div>}>
								<A href={`/post/${attachedPost}`} innerHTML={`${globalThis.location.origin}/post/${attachedPost}`} {...({ resolve: false } as any)}></A>
							</Show>
						</Show>
						<Show when={typeof attachedPost === "object"}>
							<span>&#xA0;</span>
							<A href={`/post/${attachedPost._id}`} innerHTML={`${globalThis.location.origin}/post/${attachedPost._id}`} {...({ resolve: false } as any)}></A>
						</Show>
					</p>
				</div>
			</div>
