...
const enforcedInitOptions: RequestInit = {
	credentials: "include",
	mode: "cors"
};
...
export const secureFetch = async (resource: RequestInfo | URL, init: RequestInit = {}): Promise<Response> => {
	Object.assign(init, enforcedInitOptions);
	const authToken = authStore.token;
	if (authToken) {
		const createdDate = new Date(authStore.createdAt as string);
		const expiryTimestamp = createdDate.setMilliseconds(createdDate.getMilliseconds() + parseInt(authStore.expiresIn as string, 10));
